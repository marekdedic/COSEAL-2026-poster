...
\begin{columns}[t]
	...
	\begin{column}{\colwidth}
		...
		\begin{block}{Motivation \& Experiment Setup}
			...
			\adjustbox{width=\columnwidth}{%
				\begin{tabular}{lll}
					...
					\midrule
					Grid Search, & Cora, CiteSeer, Squirrel, & GraphSAGE w/ \\
					Random Search, & PubMed, CoraFull, DBLP, & 8 hyperparameters, \\
					Bayesian Opt.\ (BO), & Computers, Flickr, & 10 evaluations, \\
		...
		\begin{block}{HPO formalisation}
			...
			\]
			given a dataset \( \mathcal{D} \), a learning algorithm \( \mathscr{F} \), a search space \( \tilde{\Lambda} \) of hyperparameter configurations, and a performance metric \( \rho \) to optimize.
			%\adjustbox{width=\columnwidth}{%
	...
	\begin{column}{\colwidth}
		...
		\begin{alertblock}{Key Takeaways}
			\begin{itemize}
				\item \textbf{SMBO is the go-to for GNN HPO}: BO and TPE consistently win; random search is a strong baseline; Sobol QMC disappoints despite theoretical appeal
				\item \textbf{Graph properties predict good hyperparameters}: Cross-RF uses dataset characteristics (homophily, structure, attribute similarity etc.) to warm-start HPO with zero evaluations on the new dataset
